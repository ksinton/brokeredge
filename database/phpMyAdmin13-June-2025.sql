...
--
-- Host: mariadb:3306
-- Generation Time: Jun 13, 2025 at 09:59 PM
...
-- Procedures
--
CREATE DEFINER=`root`@`%` PROCEDURE `GetBusinessAndImages` (IN `route_in` VARCHAR(255))   BEGIN
  DECLARE v_business_id INT;

  SELECT id INTO v_business_id
  FROM business
  WHERE route COLLATE utf8mb4_unicode_ci = route_in COLLATE utf8mb4_unicode_ci
  LIMIT 1;

  IF v_business_id IS NOT NULL THEN
    SELECT * FROM business WHERE id = v_business_id;
    SELECT url, width, height, alt FROM business_images WHERE business_id = v_business_id;
  END IF;
END$$
...
(7, 'Bountiful Bakery', 'bountiful-bakery', '4088248917', NULL, NULL, NULL, 9, 350, NULL, 2021, 5, 320000, 2, 10, 4, 2, 'Artisanal Organic Bakery Café – Turnkey Business Opportunity\r\n\r\nNow available for acquisition is a beloved local bakery café that began as a humble bagel delivery operation and has since grown into a thriving retail storefront with a loyal customer base. This turnkey business is a true labor of love, built on a foundation of scratch-made baking and a strong commitment to sustainability and local sourcing.\r\n\r\nAll baked goods are prepared fresh daily using high-quality, organic ingredients, including locally milled flour from Breadtopia, eggs from nearby farms, and dairy products from the renowned Radiance Dairy. The result is a product line that not only tastes exceptional but also supports the regional agricultural community.\r\n\r\nApproximately 100% of ingredients used are certified organic, and when unavailable, substitutes are responsibly sourced from trusted suppliers. The bakery further distinguishes itself with eco-conscious practices, including the exclusive use of compostable packaging materials and a strict avoidance of petroleum-based plastics—underscoring its mission to operate sustainably.\r\n\r\nBeyond its outstanding products, the bakery prides itself on its warm, welcoming atmosphere and dedicated team. Employees are treated like family, and this genuine care translates directly into customer satisfaction and repeat business.\r\n\r\nThis is a rare opportunity to take over a fully operational bakery café with an established brand, community goodwill, and strong growth potential. Ideal for an entrepreneur or chef passionate about quality, sustainability, and hospitality.\r\n\r\nInquire today to learn more about this profitable and purpose-driven business.', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 5, 1747850932, 1747926957);
