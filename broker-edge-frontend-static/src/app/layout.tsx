...
const layoutMap: Record<string, { Header: React.ComponentType; Footer: React.ComponentType }> = {
    ...
    "iowacitybusinessesforsale.com": {
        Header: IowaCityHeader,
        Footer: IowaCityFooter,
    },
    // Add more entries here:
    // "desmoinesbiz.com": { Header: DesMoinesHeader, Footer: DesMoinesFooter },
};
...
export default function RootLayout({
  ...
  const matched = Object.entries(layoutMap).find(([key]) => host.includes(key));

  const Header = matched?.[1].Header ?? FairfieldHeader; // default fallback
  const Footer = matched?.[1].Footer ?? FairfieldFooter;

  console.log("host", host);
  console.log("layout:", matched ? matched[0] : "default (Fairfield)");
